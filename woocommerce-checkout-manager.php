...
<thead>
	<tr>
<th style="width:5%;" class="wccs-order" title="<?php esc_attr_e( 'Change order' ); ?>"></th>
		<th>Label</th>
		<th>Placeholder</th>		
	</tr>
</thead>
...
<td class="wccs-order" title="<?php esc_attr_e( 'Change order' ); ?>"><?php echo $i + 1; ?></td>
<input type="hidden" name="<?php echo $hidden_field_name; ?>" value="Y">
 

          <td><input type="text" name="wccs_settings[buttons][<?php echo $i; ?>][label]" style="width:100%;" 
          value="<?php echo esc_attr( $options['buttons'][$i][label] ); ?>" /></td>

          <td><input type="text" name="wccs_settings[buttons][<?php echo $i; ?>][placeholder]" style="width:100%;" 
          value="<?php echo esc_attr( $options['buttons'][$i][placeholder] ); ?>" /></td>


<td class="wccs-remove"><a class="wccs-remove-button" href="javascript:;" title="<?php esc_attr_e( 'Remove Field' ); ?>">&times;</a></td>
   </tr>
<?php endfor; endif; ?>
<!-- Empty -->
...
<td class="wccs-order" title="<?php esc_attr_e( 'Change order' ); ?>"><?php echo $i; ?></td>
<input type="hidden" name="<?php echo $hidden_field_name; ?>" value="Y">

       <td><input type="text" name="wccs_settings[buttons][<?php echo $i; ?>][label]" style="width:100%;" value="" /></td>
       <td><input type="text" name="wccs_settings[buttons][<?php echo $i; ?>][placeholder]" style="width:100%;" value=" " /></td>
...
<td><a href="javascript:;" id="wccs-add-button" class="button-secondary"><?php _e( '+ Add New Field' ); ?></a></td>
...
function wccs_admin_plugin_actions($links) {
	...
	$wccs_plugin_links = array(
          '<a href="options-general.php?page=wccs_settings">'.__('Settings').'</a>',
...
if ( ! empty( $btn['label'] ) )
   ...
   woocommerce_form_field( 'wccs_field_name', array(
        'type'          => 'text',
        'class'         => array('wccs-field-class wccs-form-row-wide'), 
        'label'         => __( ''.$btn['label'].' <abbr class="required" title="required">*</abbr>'),
        'placeholder'       => __( ''.$btn['placeholder'].''),

        ), $checkout->get_value( 'wccs_field_name' )); 
...
function wccs_custom_checkout_field_update_order_meta( $order_id ) {
    ...
    if ( $_POST['wccs_field_name'])
 update_post_meta( $order_id, 'My Field', esc_attr($_POST['wccs_field_name']));
}
