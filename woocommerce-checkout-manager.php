...
<tbody>
   <tr>
<input type="hidden" name="<?php echo $hidden_field_name; ?>" value="Y">
		<td><strong><?php _e('Deselect Check boxes in this Column', 'woocommerce-checkout-manager');  ?></strong></td>
...
<input type="hidden" name="<?php echo $hidden_field_name; ?>" value="Y">
     ...
     <td><input name="wccs_settings[check][wccs_rq_3_s]" type="checkbox" class="rq_s" value="1" <?php if (  1 == ($options['check']['wccs_rq_3_s'])) echo "checked='checked'"; ?> /></td>
   </tr>
</tbody>
...
<input type="hidden" name="<?php echo $hidden_field_name; ?>" value="Y">
		<td><strong><?php _e('Deselect Check boxes in this Column', 'woocommerce-checkout-manager');  ?></strong></td>
		<td><input name="wccs_settings[check][deselect_all_rm_s]" type="checkbox" id="deselect_all_rm_s" value="1" <?php if (  1 == ($options['check']['deselect_all_rm_s'])) echo "checked='checked'"; ?> /></td>
                <td><input name="wccs_settings[check][deselect_all_rq_s]" type="checkbox" id="deselect_all_rq_s" value="1" <?php if (  1 == ($options['check']['deselect_all_rq_s'])) echo "checked='checked'"; ?> /></td>
		</tr>
</tbody>
...
<thead>
	<tr>
		<th><?php _e('Field Name', 'woocommerce-checkout-manager');  ?></th>
		<th><?php _e('New Value?', 'woocommerce-checkout-manager');  ?></th>		
	</tr>
</thead>
...
<div style="margin:20px 0 15px 10px;color:green;font-size:18px;">:: <?php _e('Add New Field Section', 'woocommerce-checkout-manager');  ?> ::</div>
...
function wccs_shipping_required_fields3( $address_fields3 ) {
$options = get_option( 'wccs_settings' );
if (  1 == ($options['check']['wccs_rq_3_s'] ) ) 
	$address_fields3['shipping_company']['required'] = false;
	return $address_fields3;
}
add_filter( 'woocommerce_shipping_fields', 'wccs_shipping_required_fields3', 10, 1 );
